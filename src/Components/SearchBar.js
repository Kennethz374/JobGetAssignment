...
export default function SearchBar() {
  ...
  const handleSearch = () => {
    axios
      .get(
        `https://api.ziprecruiter.com/jobs/v1?search=${state.job}&location=${state.city}&api_key=${API}&page=${state.page}&radius_miles=${state.miles}&days_ago=${state.date}`
      )
      .then(response => {
        setState(state => ({
          ...state,
  ...
  return (
    <>
        ...
        <form className={classes.form} onSubmit={onSubmit}>
          <div className={classes.left}>
            <FormControl fullWidth className={classes.formControl}>
              ...
              <Input type="text" id="job" name="job" autoFocus></Input>
            </FormControl>
          </div>
      ...
      {state.error === true && (
        <h1 style={centerText}>
          Opps, Something Wrong With Our Server, Please Come back later
        </h1>
      )}
      {state.data.jobs ? <Results results={state.data.jobs} /> : <Loading />}
      {state.data.total_jobs === 0 && (
        <h1 style={centerText}>Sorry! No Result Matches</h1>
      )}
      {state.data.jobs && (
        <>
          <div style={centerText}>
            <Pagination
              ...
              totalPages={
                state.data.total_jobs <= 500
                  ? Math.floor(state.data.total_jobs / 20)
                  : 25
              }
            />
          </div>
